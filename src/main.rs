...
const BENCHMARK_PERFECT_TREES: &[usize] = &[ 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432, 33554432 * 32, ];
const BENCHMARK_IMPERFECT_TREES: &[usize] = &[ 250, 500, 1000, 2000, 4000, 8000, 16000, 32000, 64000, 128000, 256000, 512000, 1024000, 2048000, 4096000, 8192000, 16384000, 32768000, 32768000 * 32, ];
...
fn print_help() {
    ...
    println!("");
    println!("Commands:");
    println!("  info              Prints device information");
    println!("  benchmark         Run benchmark");
    println!("  benchmark-tree    Run tree benchmark");
}
...
fn run_benchmark(
    device: &mut metal::Device,
    kernel: &mut metal::Function,
    sort_kernel: &mut metal::Function,
    iterations: usize,
) -> Result<(), String> {
    assert!(iterations > 0);
    ...
    for i in 0..BENCHMARK_IMPERFECT_TREES.len() * 2 {
        ...
        println!(
            "{} leaves ({} Mb), {} iterations, Sort: {}ms avg, Compute: {}ms avg)",
            leaves,
            leaves * 32 / (1024 * 1024),
            iterations,
            average_sort_time as u64,
            average_time as u64
        );
    }
    Ok(())
...
fn run_tree_benchmark(
    ...
    for i in 0..BENCHMARK_IMPERFECT_TREES.len() * 2 {
        ...
        println!(
            "{} Leaves, {} Iterations, Tree: {}ms, Prepare: {}ms avg, Compute: {}ms avg)",
