...
#define bitselect(b, a, cmp) (((a) & (cmp)) | ((b) & ~(cmp)))
...
void sha256_process2(const thread  uint  *W, thread uint *digest){
  uint a = digest[0];
  uint b = digest[1];
  uint c = digest[2];
  uint d = digest[3];
 ...
 */
void sha256d_cat(device const uint* lhs, device const uint* rhs, device uint* md){
    uint W[16]={0};
    uint state[8];
...
kernel void merkle(device uchar* data, constant uint& round, constant uint& leaves, uint id [[thread_position_in_grid]]){
    device  uint* lhs = (device  uint*)&data[SHA256_DIGEST_SIZE * (id << (round + 1))];
    device  uint* rhs = (device  uint*)&data[SHA256_DIGEST_SIZE * ((id << (round + 1)) + (1 << round))];
